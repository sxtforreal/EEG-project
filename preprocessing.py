...
## Use preprocessing_erp with plot_erp; use preprocessing_nonerp with plot_nonerp.
def preprocessing_erp(subject,which_round):
    r = mne.io.read_raw_edf('/Users/sunxiaotan/Desktop/BCI/dat/Subject' + subject + '/Session001/Train/RowColumn/Subject_' + subject + '_Train_S001R' + which_round + '.edf')
    r.load_data()
    ...
    r.set_eeg_reference('average', projection = True) #Set CAR
    ...
    ica.fit(r)
    eog_indices, eog_scores = ica.find_bads_eog(r, ch_name = ['Fp1','Fp2'], measure = 'correlation', threshold = 'auto') #Use Fp1 and Fp2 to simulate EOG channel and remove eye artifact
    ...
    epoch_r = mne.Epochs(r, events = events, event_id = event_id, tmin = tmin,
                    tmax = tmax, baseline = baseline, picks = ['F3','Fz','F4','T7','C3','Cz','C4','T8','CP3','CP4','P3','Pz','P4','PO7','PO8','Oz','Fp1','Fp2','F7','F8','FC5','FC1','FC2','FC6','CPz','P7','P5','PO3','POz','PO4','O1','O2'])
...
def plot_erp(subject, n):
    ...
    ERP_flash = A['Flash'].average()
    peak = ERP_flash.get_peak(return_amplitude = True) #Find the peak of the average signal intensity
    peak_time = peak[1] #Get the time of the peak
    intensity_at_peak = ERP_flash.get_data(tmin = peak_time)[:,0] #Get the intensities of all channels at peak time
    intensity_at_peak = np.absolute(intensity_at_peak) #Compute the absolute intensity
    zip_list = zip(list(intensity_at_peak), ERP_flash.ch_names)
    top_n = sorted(zip_list, key=lambda x: x[0], reverse = True)[:n]
    top_n = list(zip(*top_n))
    top_n_channels = list(top_n[1]) #Get the top N strong intensity channels at peak time
    ERP_flash.plot(picks = top_n_channels)
    #ERP_flash.plot_topomap(times=[0.2, 0.3, 0.4], average=0.05)
    #ERP_flash.plot(gfp='only')
    #ERP_flash.plot_joint(picks = top_n_channels, times = peak_time)
...
def preprocessing_nonerp(subject, which_round):
    ...
    ERP = mne.Epochs(r, events = epochs_without_ERP, event_id = event_id, tmin = tmin,
                    tmax = tmax, baseline = baseline, picks = ['F3','Fz','F4','T7','C3','Cz','C4','T8','CP3','CP4','P3','Pz','P4','PO7','PO8','Oz','Fp1','Fp2','F7','F8','FC5','FC1','FC2','FC6','CPz','P7','P5','PO3','POz','PO4','O1','O2'])
    return ERP

def plot_nonerp(subject, top_n_channels):
